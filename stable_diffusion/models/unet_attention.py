...
class CrossAttention(nn.Module):
    ...
    def flash_attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor):
        ...
        if self.d_head <= 32:
            ...
            raise ValueError(f"Head size ${self.head} too large for Flash Attention")

        if pad:
            qkv = torch.cat((qkv, qkv.new_zeros(batch_size, seq_len, 3, self.n_heads, pad)), dim=-1)
        
        out, _ = self.flash(qkv) # [batch_size, seq_len, n_heads, d_padded]
        out = out[:, :, :, :self.d_head]
        out = out.reshape(batch_size, seq_len, self.n_heads, self.d_head)
        return self.to_out(out)

    def normal_attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor):
        q = q.view(*q.shape[:2], self.n_heads, -1) # [batch_size, seq_len, n_heads, d_head]
        k = k.view(*k.shape[:2], self.n_heads, -1)
        v = v.view(*v.shape[:2], self.n_heads, -1)
        attn = torch.einsum('bhij,bjhd -> bihd', q, k) * self.scale
